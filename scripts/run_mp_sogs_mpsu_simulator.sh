...
OUT_FILE="${OUT_FILE:-${OUT_DIR}/mp_sogs_mpsu_sim_${TIMESTAMP}.csv}"
...
echo "[mp-sogs-sim] running simulator -> ${OUT_FILE}"
java --add-modules jdk.incubator.vector \
  -Dlog4j.configuration="file:${LOG4J_FILE}" \
  -cp "${FULL_CP}" \
  edu.alibaba.mpc4j.s2pc.pso.mpsu.sogs.MpSogsMpsuSimulatorMain "$@" \
  | tee "${OUT_FILE}"
