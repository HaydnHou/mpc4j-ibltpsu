...
public class Rep4PrssSecureMpSogsUnionPeel implements SecureMpSogsUnionPeel {
    ...
    public Rep4PrssSecureMpSogsUnionPeel(Rpc rpc, MpSogsSketch localSketch, MpSogsMpsuParams params, long taskId,
        ...
        session = new Rep4PrssSession(rpc, taskId);
    ...
    public BatchMpSogsPeelOutput peelBatch(BatchMpSogsPeelInput input) {
        ...
        Rep4PrssPackedBooleanBackend backend = session.createBackend(input.size(), PrssPhase.FULL);
    ...
    private PackedBooleanShare[] shareWire(long[] wire, Rep4PrssPackedBooleanBackend backend) {
    ...
    private LocalPackedWires encodeLocalWires(BatchMpSogsPeelInput input,
                                               Rep4PrssPackedBooleanBackend backend) {
        int blockNum = backend.blockNum();
        int expectedBlockNum = (input.size() + Long.SIZE - 1) / Long.SIZE;
        if (blockNum != expectedBlockNum) {
            throw new IllegalStateException("REP4 PRSS actual-size block mismatch: " + blockNum
                + " != " + expectedBlockNum);
        }
