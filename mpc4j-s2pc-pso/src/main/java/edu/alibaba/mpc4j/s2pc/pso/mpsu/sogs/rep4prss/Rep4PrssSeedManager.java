...
/**
 * PRSS seed manager for 4-party replicated packed Boolean sharing.
 *
...
class Rep4PrssSeedManager {
    ...
    private final Rep4PrssRandomSource randomSource;

    Rep4PrssSeedManager(Rpc rpc, long taskId) {
        this.rpc = rpc;
        this.taskId = taskId;
        ...
        if (parties.length != PARTY_NUM) {
            throw new IllegalArgumentException("REP4 PRSS requires exactly 4 parties: " + parties.length);
        }
        ownPartyId = rpc.ownParty().getPartyId();
        componentSeeds = new byte[PARTY_NUM][];
        randomSource = new Rep4PrssRandomSource(taskId);
        setupSeeds();
    }
        ...
        for (int componentId = 0; componentId < PARTY_NUM; componentId++) {
            ...
            if (ownPartyId == ownerId) {
                ...
                if (payload.size() != 1 || payload.get(0).length != SEED_BYTE_LENGTH) {
                    throw new IllegalStateException("invalid REP4 PRSS seed payload for component " + componentId);
                }
