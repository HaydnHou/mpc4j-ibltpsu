...
/**
 * 4-party PRSS packed replicated Boolean share.
 *
 * <p>The secret is represented as {@code x = r0 xor r1 xor r2 xor r3}. Party {@code Pi} holds every component except
 * {@code ri}. Missing local components are stored as {@code null}.</p>
 *
 * @author donghai hou
 * @date 2026/06/22
 */
public class Rep4PrssPackedBooleanShare extends PackedBooleanShare {
    /**
     * Party number.
     */
    static final int PARTY_NUM = 4;
    /**
    ...
    public Rep4PrssPackedBooleanShare(long[][] components, int blockNum) {
        super(new long[blockNum]);
        this.blockNum = blockNum;
        if (components.length != PARTY_NUM) {
            throw new IllegalArgumentException("REP4 PRSS share must have 4 components");
        }
        ...
        for (int componentIndex = 0; componentIndex < PARTY_NUM; componentIndex++) {
            if (components[componentIndex] != null && components[componentIndex].length != blockNum) {
                throw new IllegalArgumentException("invalid REP4 PRSS component block length");
            }
