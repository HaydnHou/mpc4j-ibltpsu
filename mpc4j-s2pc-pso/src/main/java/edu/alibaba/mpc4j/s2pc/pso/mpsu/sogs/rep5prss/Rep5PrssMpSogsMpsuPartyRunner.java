...
/**
 * Party-local REP5 PRSS packed MP-SOGS MPSU runner.
 *
 * @author donghai hou
 * @date 2026/06/22
 */
public class Rep5PrssMpSogsMpsuPartyRunner {
    /**
    ...
    public Rep5PrssMpSogsMpsuPartyRunner(Rpc rpc, MpSogsMpsuConfig config, long taskId) {
        this.rpc = rpc;
        this.config = config;
        this.taskId = taskId;
        if (config.getSecurePeelType() != MpSogsMpsuConfig.SecurePeelType.REP5_PRSS_OPENED_FIRST) {
            throw new IllegalArgumentException("REP5 PRSS runner requires REP5_PRSS_OPENED_FIRST secure peel type");
        }
        if (config.getParams().getPartyNum() != Rep5PrssPackedBooleanBackend.PARTY_NUM) {
            throw new IllegalArgumentException("REP5 PRSS runner requires exactly 5 parties");
        }
    ...
    private boolean openAnyResidual(MpSogsSketch localSketch) {
        Rep5PrssPackedBooleanBackend backend = new Rep5PrssPackedBooleanBackend(rpc, 1, taskId + 0x4E17_0000L);
        long[] localResidual = new long[]{localSketch.getRemainingElements().isEmpty() ? 0L : 1L};
        Rep5PrssPackedBooleanShare[] sharesByParty = backend.shareOwnAndReceiveAll(localResidual);
        PackedOrAccumulator accumulator = new PackedOrAccumulator(backend);
        for (Rep5PrssPackedBooleanShare share : sharesByParty) {
    ...
    private static class PackedOrAccumulator {
        ...
        private PackedOrAccumulator(Rep5PrssPackedBooleanBackend backend) {
            this.backend = backend;
            value = backend.zero();
        }

        private void orIn(Rep5PrssPackedBooleanShare share) {
