...
/**
 * REP4 packed MP-SOGS MPSU tests.
 *
 * @author donghai hou
 * @date 2026/06/22
 */
public class Rep4MpSogsMpsuPartyRunnerTest {
    @Test
    public void testFourPartyAllOutput() throws InterruptedException {
        ...
        Rep4Thread[] threads = IntStream.range(0, 4)
            .mapToObj(partyIndex -> new Rep4Thread(rpcs[partyIndex], inputs.get(partyIndex), expectedUnion, config))
        ...
        try {
            ...
            for (Rep4Thread thread : threads) {
                if (thread.isAlive()) {
                    thread.interrupt();
                    Assert.fail("REP4 MP-SOGS thread timed out: " + thread.getName());
                }
                if (thread.throwable != null) {
                    throw new AssertionError("REP4 MP-SOGS party failed: " + thread.getName(), thread.throwable);
                }
    ...
    private static class Rep4Thread extends Thread {
        ...
        private Rep4Thread(Rpc rpc, Set<Long> localInput, Set<Long> expectedUnion, MpSogsMpsuConfig config) {
            this.rpc = rpc;
            this.localInput = localInput;
            this.expectedUnion = expectedUnion;
            this.config = config;
            setName("rep4-mp-sogs-party-" + rpc.ownParty().getPartyId());
        }

        @Override
        public void run() {
            try {
                transcript = new Rep4MpSogsMpsuPartyRunner(rpc, config, 9_000_000L)
