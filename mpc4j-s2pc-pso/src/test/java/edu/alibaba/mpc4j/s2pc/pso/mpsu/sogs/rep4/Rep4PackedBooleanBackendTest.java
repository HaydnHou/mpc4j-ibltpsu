...
/**
 * REP4 packed Boolean backend tests.
 *
 * @author donghai hou
 * @date 2026/06/22
 */
public class Rep4PackedBooleanBackendTest {
    @Test
    ...
    private void runBackendTest(int batchSize) throws InterruptedException {
        ...
        MemoryRpcManager rpcManager = new MemoryRpcManager(Rep4PackedBooleanBackend.PARTY_NUM);
        Rpc[] rpcs = IntStream.range(0, Rep4PackedBooleanBackend.PARTY_NUM)
            .mapToObj(rpcManager::getRpc)
            .toArray(Rpc[]::new);
        Arrays.stream(rpcs).forEach(Rpc::connect);
        Rep4Thread[] threads = IntStream.range(0, Rep4PackedBooleanBackend.PARTY_NUM)
            .mapToObj(partyIndex -> new Rep4Thread(rpcs[partyIndex], batchSize, inputs, lastMask))
            .toArray(Rep4Thread[]::new);
        try {
            Arrays.stream(rpcs).forEach(Rpc::reset);
            Arrays.stream(threads).forEach(Thread::start);
            for (Rep4Thread thread : threads) {
                thread.join(30_000L);
            }
            for (Rep4Thread thread : threads) {
                if (thread.isAlive()) {
                    thread.interrupt();
                    Assert.fail("REP4 backend thread timed out: " + thread.getName());
                }
                if (thread.throwable != null) {
                    throw new AssertionError("REP4 backend thread failed: " + thread.getName(), thread.throwable);
                }
            }
        } finally {
    ...
    private static long[][] randomInputs(int batchSize, int blockNum, long lastMask) {
        Random random = new Random(20260622L + batchSize);
        long[][] inputs = new long[Rep4PackedBooleanBackend.PARTY_NUM][blockNum];
        for (int partyIndex = 0; partyIndex < Rep4PackedBooleanBackend.PARTY_NUM; partyIndex++) {
    ...
    private static class Rep4Thread extends Thread {
        ...
        private Rep4Thread(Rpc rpc, int batchSize, long[][] inputs, long lastMask) {
            this.rpc = rpc;
            this.batchSize = batchSize;
            this.inputs = inputs;
            this.lastMask = lastMask;
            setName("rep4-packed-backend-party-" + rpc.ownParty().getPartyId());
        }
        ...
        public void run() {
            try {
                ...
                Rep4PackedBooleanBackend backend = new Rep4PackedBooleanBackend(rpc, batchSize, 8_000_000L);
                Rep4PackedBooleanShare[] shares = backend.shareOwnAndReceiveAll(inputs[ownPartyId]);
                for (int dealerId = 0; dealerId < Rep4PackedBooleanBackend.PARTY_NUM; dealerId++) {
